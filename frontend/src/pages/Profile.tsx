    ...
    <DefaultLayout>
        <Breadcrumb pageName="Profile" />
            <div className="bg-white dark:bg-boxdark shadow-lg rounded-lg p-6 space-y-4 border border-stroke dark:border-strokedark">
                ...
                <Link to={'/auth/changepassword/' + user_id}>
                    <button className="mt-4.5 w-full sm:w-auto flex items-center justify-center gap-1 rounded-lg border border-primary bg-primary py-2 px-4 text-center font-medium text-white transition hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                    Change Password
                    </button>
                </Link>
            </div>
            <div className="mt-10.5 bg-white dark:bg-boxdark shadow-lg rounded-lg p-6 space-y-4 border border-stroke dark:border-strokedark">
                ...
                <div className="space-y-2 text-base">
                    ...
                    <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-bold">Address: </span> <span className="font-thin">{currUserInfo?.email}</span> {/*Change after backend */}
                    </p>
                </div>

                <Link to={'/employee/details/' + currUserInfo?.employee_id}>
                    <button className="mt-4.5 w-full sm:w-auto flex items-center justify-center gap-1 rounded-lg border border-primary bg-primary py-2 px-4 text-center font-medium text-white transition hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
            ...
            <div className="mt-10.5 bg-white dark:bg-boxdark shadow-lg rounded-lg p-6 space-y-4 border border-stroke dark:border-strokedark">
                ...
                <div className="space-y-2 text-base">
                    ...
                    <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-bold">Supervisor:</span> <span className="font-thin">{currUserInfo?.employee_id}</span> {/*Change after backend */}
                    </p>
                    <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-bold">Paygrade:</span> <span className="font-thin">{currUserInfo?.job_title}</span> {/*Change after backend */}
                    </p>
                    <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-bold">Job Title:</span>  <span className="font-thin">{currUserInfo?.job_title}</span>
                    </p>
                    <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-bold">Employment Status:</span> <span className="font-thin">{currUserInfo?.job_title}</span> {/*Change after backend */}
                    </p>
                </div>
            </div>
