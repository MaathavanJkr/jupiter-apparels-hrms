...
import { getDependentByID, updateDependent, deleteDependent } from "../../services/dependentServices";
import { notifyError, notifySuccess } from "../../services/notify";
import { ToastContainer } from "react-toastify";

const DependentTable = ({ employee_id }: { employee_id: string }) => {
    ...
    useEffect(() => {
        const fetchDepedents = async () => {
            try {
                const dependents = await getDependentByID(employee_id);
                setDependents(dependents);
            } catch (error) {
                console.log("Failded to fetch dependents:", error);
            };
        }
    ...
    const handleDeleteDependent = () => {
        if (dependentId !== '') {
            deleteDependent(dependentId)
                .then(() => {
                    notifySuccess("Dependent Deleted");
                    setModalOpen(false);
                }).catch((error) => {
            ...
            notifyError("Please select a dependent to delete");
        }
    }
    ...
    const handleEditModalOpen = (dependent_id: string) => {
        const currDependent: Dependent = dependents?.find(dependent => dependent.dependent_id === dependent_id)!;
        setAction('Edit');
        if (currDependent && currDependent.dependent_id) {
            setDependentId(currDependent.dependent_id);
            setEmployeeId(currDependent.employee_id);
            setRelationship(currDependent.relationship_to_employee);
            setName(currDependent.name);
            setBirthdate(currDependent.birth_date);
            setModalOpen(true);
        } else {
            notifyError("Dependent not Found");
        }
    }

    const handleDeleteModalOpen = (dependent_id: string) => {
        const currDependent: Dependent = dependents?.find(dependent => dependent.dependent_id === dependent_id)!;
        setAction('Delete');
        if (currDependent && currDependent.dependent_id) {
            setDependentId(currDependent.dependent_id);
        ...
        <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                                        ...
                                        <tr key={key}>
                                            ...
                                            <td rowSpan={1} className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                                <p className="text-black dark:text-white">
                                                    {dependent.relationship_to_employee}
                                                </p>
                                            </td>
                                            ...
                                            {isAuthorized && (
                                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                                    <div className="flex items-center justify-center space-x-3.5">
                                                        <button onClick={() => handleEditModalOpen(dependent.dependent_id)} className="hover:text-primary">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                                                <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                                            </svg>
                                                        </button>
                                                        <button onClick={() => handleDeleteModalOpen(dependent.dependent_id)} className="hover:text-primary">
                                                            <svg
            ...
            <div className={`fixed left-0 top-0 z-999999 flex h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5 overflow-y-auto ${!modalOpen && 'hidden'}`}>
                <div className="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 dark:bg-boxdark md:px-17.5 md:py-15 max-h-screen overflow-y-auto">
                    ...
                    {action == "Delete" && (
                        <>
                            <div className="mb-4.5">Confirm to delete Dependent: {name}</div>
