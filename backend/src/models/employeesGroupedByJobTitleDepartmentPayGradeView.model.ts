...
export const getEmployeesGroupedByJobTitleModel = async (): Promise<Output> => {
    try {
        const [result] = await db.promise().query("CALL GetEmployeesGroupedByJobTitle()");
        return { data: result as EmployeesGroupedByJobTitle[], error: null, message: null };
    } catch (error) {
        return {
            data: null,
            error,
...
export const getEmployeesGroupedByJobTitleIDModel = async (jobTitle: string): Promise<Output> => {
    try {
        const [result] = await db.promise().query("CALL GetEmployeesGroupedByJobTitleID(?)", [jobTitle]);
        return { data: result as EmployeesGroupedByJobTitle[], error: null, message: null };
    } catch (error) {
        return {
            data: null,
