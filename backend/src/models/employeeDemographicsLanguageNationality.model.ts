...
export const getEmployeeDemographicsModel = async (): Promise<Output> => {
    try {
        ...
        return { data: result as EmployeeDemographics[], error: null, message: null };
    } catch (error) {
        return {
            data: null,
            error,
...
export const getEmployeeDemographicsByLangAndNatModel = async (nationality: string, language: string): Promise<Output> => {
    try {
        const [result] = await db.promise().query("CALL GetEmployeeDemographicsByLangAndNat(?, ?)", [nationality, language]);
        return { data: result as EmployeeDemographics[], error: null, message: null };
    } catch (error) {
        return {
            data: null,
