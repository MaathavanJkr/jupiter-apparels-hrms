...
import {
  ...
  getEmployeeBasicInfoByUserIDModel,
...
export const getEmployeeBasicInfoByUserID = async (req: Request, res: Response) => {
  const { user_id } = req.params;

  await getEmployeeBasicInfoByUserIDModel(user_id)
    .then((result) => {
      if (!result.data) {
        return res.status(404).json({ message: "Employee not found" });
      }
