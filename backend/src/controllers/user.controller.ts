...
export const getUserByID = async (req: Request, res: Response) => {
  ...
  try {
    const [users] = await db
      .promise()
      .query<User[]>(
        "SELECT user_id, employee_id, role, username FROM users WHERE user_id = ?",
        [id]
      );
    if (users.length === 0) {
...
export const updateUser = async (req: Request, res: Response) => {
  ...
  try {
    const [result] = await db
      .promise()
      .query("UPDATE users SET role=?, username=? WHERE user_id = ?", [
        role,
        username,
        id,
      ]);
...
export const deleteUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Database query failed", message: error });
  }
};
